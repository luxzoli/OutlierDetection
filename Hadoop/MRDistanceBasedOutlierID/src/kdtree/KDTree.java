...
public class KDTree {
	...
	private static ArrayList<KDTree> leaves;
	static {
		leaves = new ArrayList<KDTree>();
	}
	...
	public static void epsilonNeighborhood(KDTree node, Point p, float epsilon,
		...
		rangeSearch(node, lowerBoundary, upperBoundary, pointsInRange);
	...
	public static float kNNQuery(KDTree node, Point p, int k,
		...
		PointEntry[] pes = (PointEntry[]) pq.toArray();
		for (PointEntry pe : pes) {
			kNearestNeighbors.add(pe.getPoint());
		}
		return kDistance;
	}
	...
	public static boolean epsilonNeighborhoodCheck(KDTree node,Point p, int k, float epsilon){
		return epsilonNeighborhoodCheck(node, p, epsilon, 0) >= k;
	}
	
	private static int epsilonNeighborhoodCheck(KDTree node,Point p, float epsilon,int level){
		int inRange = 0;
		float[] lowerBoundary = node.lowerBoundary;
		float[] upperBoundary = node.upperBoundary;
		float distanceToLower = Point.euclideanDistance(lowerBoundary, p);
		float distanceToUpper = Point.euclideanDistance(upperBoundary, p);
		if(distanceToUpper <= epsilon && distanceToLower <= epsilon){
			return node.size;
		}
		int d = node.lowerBoundary.length;
		if (upperBoundary[level] < node.lowerBoundary[level]) {
		...
		if (node.isLeaf) {
			for (Point point : node.getPoints()) {
				if (Point.euclideanDistance(point, p) <= epsilon) {
					inRange++;
				}
			}
		} else {
			inRange += epsilonNeighborhoodCheck(node.left,p, epsilon,
					(level + 1) % d);
			inRange += epsilonNeighborhoodCheck(node.right,p,epsilon,
					(level + 1) % d);
		}
		return inRange;
	}
	
	public static boolean kDistanceCheck(KDTree node, Point p, int k, float kDistanceLimit) {
		float kDistance = Float.MAX_VALUE;
		PriorityQueue<PointEntry> pq = new PriorityQueue<PointEntry>(k,
				new PointEntryComparator());
		return kDistanceCheck(node, p, k, kDistance,kDistanceLimit, pq, 0) <= kDistanceLimit;
	}
	
	private static float kDistanceCheck(KDTree node, Point p, int k, float kDistance,float kDistanceLimit,
		...
		float distanceToSplit = (float) Math.sqrt(((pLevCoord - splitCoord)
				* (pLevCoord - splitCoord)));
		boolean leftCloser = pLevCoord <= splitCoord ? true : false;
		if (leftCloser) {
			kDistance = kDistanceCheck(node.left, p, k, kDistance,kDistanceLimit, kNearestNeighbors,
					(level + 1) % d);
			if(kNearestNeighbors.size() >= k && kDistanceLimit >= kDistance){
				return kDistance;
			}
			if (distanceToSplit <= kDistance
					|| kNearestNeighbors.size() < k) {
				kDistance = kDistanceCheck(node.right, p, k, kDistance,kDistanceLimit,
						kNearestNeighbors, (level + 1) % d);
			}
		} else {
			kDistance = kDistanceCheck(node.right, p, k, kDistance,kDistanceLimit, kNearestNeighbors,
					(level + 1) % d);
			if(kNearestNeighbors.size() >= k && kDistanceLimit >= kDistance){
				return kDistance;
			}
			if (distanceToSplit <= kDistance
					|| kNearestNeighbors.size() < k) {
				kDistance = kDistanceCheck(node.left, p, k, kDistance,kDistanceLimit,
						kNearestNeighbors, (level + 1) % d);
			}
		}
		return kDistance;
	...
	private static float kNNQuery(KDTree node, Point p, int k, float kDistance,
		...
		if (leftCloser) {
			...
			kDistance = kNNQuery(node.right, p, k, kDistance, kNearestNeighbors,
					(level + 1) % d);
			if (distanceToSplit <= kDistance
					|| kNearestNeighbors.size() < k) {
