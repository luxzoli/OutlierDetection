...
public class DensityBasedOutliersDriver extends Configured implements Tool {

	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(),
				new DensityBasedOutliersDriver(), args);
		System.exit(res);
	}
	...
	public int run(String[] args) throws Exception {
		...
		job.setJarByClass(DensityBasedOutliersDriver.class);
		job.setJobName("Density-based Outlier Detection");
		FileInputFormat.setInputPaths(job, new Path(inputDir));
		FileOutputFormat.setOutputPath(job, new Path(outputDir));
		job.setMapperClass(DensityBasedOutliersMapper.class);
		job.setReducerClass(DensityBasedOutliersReducer.class);
